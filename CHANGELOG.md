...
### Fixed
- **Critical: Widget Toggle Not Working** - Fixed widget toggle buttons not actually toggling servers. Sandboxed apps cannot receive `userInfo` from `DistributedNotificationCenter`, so the toggle data was being lost. Now stores pending toggle in shared UserDefaults and reads it in the main app.
