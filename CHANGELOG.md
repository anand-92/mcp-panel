...
### Added
- **Full Appearance Customization** - Settings now gives you real control over how the app looks, with everything applying live as you change it. **Liquid Glass** can be set to Regular, Clear, or Off (fully solid), with an optional accent tint and interactive glass that reacts to the pointer. **Transparency** has separate panel and window opacity sliders. **Color scheme** can follow the system or be pinned to Light or Dark. **Layout** offers Compact / Standard / Comfortable density presets plus individual corner radius, card padding, grid spacing, and card width sliders. **Typography** covers text size, interface font (Poppins / System / Serif), code font size, and JSON preview blur strength. **Motion** has four levels (Off / Subtle / Standard / Playful) along with hover-effect and shadow toggles. Accessibility opt-outs for **Honor Reduce Transparency** and **Honor Reduce Motion** are on by default and warn you when a system setting is overriding your choice.
- **Inspector View** - A third view mode alongside Grid and JSON: a dense server list on the left with an **inline enable switch on every row**, and a tabbed detail pane on the right. Toggling from the list never changes the selection, so you can flip servers on and off at a glance while keeping the selected server's details open. The detail pane has three tabs: **Overview** (transport, command, arguments, endpoint, working directory, tags, health check, and a large editable icon), **JSON** (full-height syntax-highlighted editor with Format / Revert / Save, supporting rename via the top-level key), and **Environment** (a real key/value editor for `env` variables and HTTP `headers`, with values masked until revealed). Unsaved edits are protected by a discard confirmation when switching servers, and the sidebar footer shows an enabled count.
- **Last-Toggled Server Ordering** - The All page now keeps the most recently toggled servers at the top, including toggles from the app, menu bar, and `mcp-panel` CLI.
- **Standard `mcp.json` Onboarding** - First-time setup can now select a folder containing an existing `mcp.json`, so MCP Panel can manage Factory Droid and other MCP-compatible apps without requiring a Claude Code config. The app securely retains access to the selected folder for reading, writing, and live reload.
- **CLI mirrors the Factory (Droid) config** - `mcp-panel` is now fully 1:1 with the GUI across both config surfaces. `add` and `toggle` write the enabled set to the Factory config (`~/.factory/mcp.json`) in addition to `~/.claude.json`, normalized exactly as the app's `syncClaudeServersToDroid` does (explicit `type`, `httpUrl`→`url`, flattened `transport`). `list` now reports `factoryConfigPath` and `factoryInSync`. The Factory path is read from the app's `droidConfigPath` setting; override with `--factory <path>` or `MCP_PANEL_FACTORY_CONFIG`. When no Factory path is configured, Droid sync stays disabled — matching the GUI.
- **Install Command-Line Tool from the App** - The `mcp-panel` CLI now ships *inside* the app bundle (direct-download / DMG builds), and a new **Install Command-Line Tool…** item in the app menu symlinks it into `~/.local/bin`. Because it's a symlink into the bundle, Sparkle updates keep the CLI current automatically. The DMG build pipeline builds, embeds, and code-signs the binary; the menu item is hidden in the sandboxed Mac App Store build, which can't ship or install a CLI.
...
### Changed
- **Settings Rebuilt Around Live Editing** - The Settings modal was rewritten as a six-tab surface (General, Theme, Glass, Layout, Motion, Advanced) with live preview cards on the Glass and Layout tabs that render a miniature server card over a checkerboard, so translucency is actually visible while you drag a slider. The **Save button is gone** — every change applies immediately across the whole app; config path fields still commit on Return or when they lose focus, since each commit restarts the file watcher. Each section has its own Reset, plus a Reset Appearance action for everything at once.
- **Now Requires macOS 26 (Tahoe)** - The deployment target moved from macOS 13 to macOS 26 so the app can use the current SwiftUI feature set unconditionally. Liquid Glass (`glassEffect`) is now applied directly instead of behind an availability check, and the macOS 13–25 glass-morphism fallback, the legacy gradient window background, and the `NSApp.activate(ignoringOtherApps:)` fallback have been removed. Apple requires App Store uploads to be built with the macOS 26 SDK or later as of April 28, 2026.
- **Swift 6 Language Mode** - The package now builds with swift-tools-version 6.4 in the Swift 6 language mode with complete strict-concurrency checking, and the Xcode project matches (`SWIFT_VERSION 6.0`, `SWIFT_STRICT_CONCURRENCY complete`, Xcode 27). The shared services (`ConfigManager`, `BookmarkManager`, `CustomIconManager`) are now `Sendable`, the config models are `Sendable` value types, `UpdateChecker` and the JSON editor's coordinator are main-actor isolated, and `ConfigFileWatcher` documents its serial-queue invariant. Data-race safety is now enforced at compile time rather than by convention.
- **Modernized SwiftUI APIs** - Replaced every deprecated `onChange(of:perform:)` call with the two- or zero-parameter form, and migrated all 134 `foregroundColor` calls to `foregroundStyle`. The drag-and-drop pipeline moved off the deprecated `NSItemProvider.loadItem` callback style onto async wrappers in a new `DroppedItemLoader`, and App Store build detection no longer uses the deprecated `appStoreReceiptURL`.
- **Sparkle 2.9.5** - Updated the updater framework from the pinned 2.9.2 to the latest 2.9.5, in both the SwiftPM manifest and the Xcode project pin.
- **Minimal Hardened-Runtime Entitlements** - The DMG entitlements no longer request `allow-jit`, `allow-unsigned-executable-memory`, `disable-library-validation`, `allow-dyld-environment-variables`, or `disable-executable-page-protection`. A pure Swift/SwiftUI app needs none of them, and Developer ID signing already satisfies Sparkle's library validation, so the app now ships with hardened runtime enabled and zero exceptions. The App Store entitlements dropped the redundant `files.user-selected.read-only` key, which `read-write` already implies.
- **Icon-Only View Switcher** - The toolbar's view-mode control now shows icons with tooltips instead of icons plus labels. Three labeled pills would have pushed the toolbar past the window's 900 pt minimum width; the mode name is preserved in the tooltip and the accessibility label.
...
### Fixed
- **Light Themes Actually Render Light** - The app hardcoded a dark color scheme on the root scene, so the Solarized Light theme drew light surface colors inside a dark environment and never looked right. The color scheme is now driven by the appearance setting, with a matching `NSAppearance` applied to the window chrome and the menu bar popover.
- **Live Reload on Focus** - The app now reloads the config from disk when it returns to the foreground, so external edits — including writes from the `mcp-panel` CLI — show up without pressing ⌘R. The descriptor-based file watcher can miss an atomic file replace of the sandboxed config (a temp file `rename()`d over the original swaps the inode); reloading on app activation closes that gap. It reuses the existing self-write suppression window, so the app's own saves don't trigger redundant reloads.
...
### Removed
- **Embedded `mcp-panel` CLI** - Removed the agent CLI entirely: the `MCPPanelCLI` SwiftPM target and `mcp-panel` executable product, the in-app **Install Command-Line Tool…** menu item and `CLIInstaller`, the DMG pipeline steps that built/embedded/signed the binary, and the bundled `skills/mcp-panel-cli/` agent skill. The package is back to a single `MCPServerManager` executable, so bare `swift run` works again. Config management is now GUI-only.
