...
struct MiniModeView: View {
    ...
    private var miniHeader: some View {
        HStack(spacing: 8) {
            ...
            // Active config indicator (clickable to switch)
            Button(action: {
                withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) {
                    viewModel.settings.activeConfigIndex = viewModel.settings.activeConfigIndex == 0 ? 1 : 0
                }
            }) {
                Text(viewModel.settings.activeConfigIndex == 0 ? "Claude" : "Gemini")
            ...
            .buttonStyle(.plain)
            .help("Switch to \(viewModel.settings.activeConfigIndex == 0 ? "Gemini" : "Claude")")
...
struct MiniServerRow: View {
    ...
    var body: some View {
            ...
            Button(action: onToggle) {
                ZStack {
                    Capsule()
                        ...
                        .overlay(
                            Capsule()
                                .stroke(isEnabled ? Color.clear : themeColors.borderColor, lineWidth: 0.5)
                        )
        ...
        .background(
            RoundedRectangle(cornerRadius: 6)
                .fill(isHovering ? themeColors.glassBackground : Color.clear)
        )
        .onHover { hover in
