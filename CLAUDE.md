...
```
Config 1: ~/.claude.json  (Claude Code) - JSON
Config 2: ~/.settings.json (Gemini CLI) - JSON
```
...
```
Models/
  - ServerModel.swift      # Server data model with inConfigs array
  - ServerConfig.swift     # MCP server structure (stdio/http/sse transport)
  - ConfigFormat.swift     # Config format (JSON)
...
Views/Modals/
  - AddServerModal.swift   # For adding servers (JSON, has registry browser)
  - SettingsModal.swift    # Manages both config paths
```
